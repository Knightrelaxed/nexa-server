...
-- ══════════════════════════════════════════════════════════════
('[FALLBACK ENGINE 11-TIER DETAIL] [FALLBACK] Fallback_Engine.js executes AI calls through 11 tiers in this exact order: Tier 1-4 = Groq meta-llama/llama-4-scout-17b-16e-instruct (4 separate API keys, ~500ms avg, 30k TPM limit). Tier 5-8 = Gemini 2.5 Flash (deep reasoning & infinite quota, 4 separate keys). Tier 9 = Cerebras gemma-4-31b (ultra-fast backup). Tier 10 = Mistral Pixtral 12B. Tier 11 = OpenRouter Multi-Model Free (loops over 5 free models: gemma-4-31b, gemma-4-26b, llama-3.3-70b, qwen3-80b, lfm-2.5-1.2b). Each tier is tried with internal retries before dropping to the next tier.'),
