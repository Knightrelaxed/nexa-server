...
async function routeUserMessage(textInput, runtimeHints = {}) {
  ...
  try {
    ...
    if (categoriesResult.status === 'fulfilled' && categoriesResult.value && categoriesResult.value.length > 0) {
      ...
      activeCategoriesBlock = `\n[KATEGORI TRANSAKSI AKTIF — DAFTAR MUTLAK UNTUK FIELD "category" DI FINANCE]\n${_catLines.join('\n')}\n\n[PANDUAN PEMILIHAN KATEGORI (SUPER STRICT)]\n1. KETEPATAN KARAKTER: Anda HANYA BOLEH menyalin salah satu nama kategori dari daftar di atas secara PERSIS (huruf besar/kecil, spasi, simbol). DILARANG KERAS mengarang kategori yang tidak ada di daftar (misalnya dilarang memakai "Makanan & Minuman" atau "Perawatan & Kecantikan" jika itu tidak ada di daftar).\n2. PENALARAN SEMANTIK: Tanyakan "Apa SUBSTANSI/OBJEK yang dibeli?" lalu cari padanan terdekatnya HANYA di daftar aktif.\n- Makanan/Minuman: Jika membeli nasi, ayam, sate, dll, gunakan "Makan Berat / Makan Luar". Jika membeli camilan/kopi, gunakan "Jajan / Ngopi / Kafe".\n- Jasa/Layanan: Jika bayar laundry/cuci baju, gunakan "Jasa Laundry".\n- Belanja: Jika beli sabun/beras di minimarket, gunakan "Bahan Makanan / Groceries".\n- Transportasi: Grab/Gojek gunakan "Ojek / Taksi Online" atau "Transportasi Umum".\n- Jika benar-benar tidak ada kategori spesifik yang cocok, gunakan "Lainnya" (jika tersedia).\n`;
    }
