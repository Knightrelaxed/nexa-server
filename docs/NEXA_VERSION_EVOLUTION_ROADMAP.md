...
graph TD
    ...
    subgraph Azure Cloud - Indonesia Central Jakarta
        CAD[Caddy HTTPS Reverse Proxy - Let's Encrypt SSL]
        
        subgraph VM Standard_B2ats_v2 Ubuntu 24.04 ARM
            PM2[PM2 Process Manager - Systemd Auto-Revive]
            CORE[N.E.X.A Core Server v3.0.0 - Node.js 20 Production]
            CRON[The Pulse Engine - 15 Proactive Background Jobs]
        end
    end

    subgraph External Neural & Data Cloud
        SUP[(Supabase PostgreSQL ACID)]
        GEM[Google Gemini 2.5 / Flash AI]
        GROQ[Groq Llama 3.3 70B Ultra-Fast]
        GWRK[Google Tasks & Workspace API]
        PMD[PM2 Plus Cloud Monitoring]
    end
